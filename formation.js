$(function () 
{
    ...
    var css3_animation_end = [ 'webkitTransitionEnd', // webkit(Chrome1.0, Safari3.2)
    ...
    $.extend( formation.prototype, 
    {
        ...
        option : 
        {
            first_mode_name : 'normal_mode', css_class_prefix : '-fm-', debug_flg : false, history_cnt : 5 
        }
    });
    ...
    function set_styles()
    {
        ...
        {
            var rules = (document.styleSheets[styleSheetsNum]['rules'] != undefined) ? document.styleSheets[styleSheetsNum]['rules'] : document.styleSheets[styleSheetsNum]['cssRules'];
    ...
    function set_mode_bank(_t, _mode_piece) 
    {
        ...
        if (_t.mode[element_name] == undefined) 
        {
            _t.mode[element_name] = {
                'mode' : _t.option.first_mode_name, 'bank' : mode_piece, 'history' : [], 'com' : {}
            };
            return true;
        }
        else 
    ...
    function add_mode_history(exe, element_name, one_mode_styles, one_mode_styles_max_index) 
    {
        ...
        if ( exe.option.first_mode_name == cap_mode_name) 
        {
            for (var one_mode_style_index = 0; one_mode_style_index <= one_mode_styles_max_index; one_mode_style_index++) 
            {
                ...
                {
                    if (add_mode_history_obj[cap_mode_name][element] == undefined) {
                        add_mode_history_obj[cap_mode_name][element] = {};
                    }
                    if (typeof (styles[element]) == 'function')
                    {
                        styles[element] = styles[element]();
                    }
                    add_mode_history_obj[cap_mode_name][element] = $.extend(add_mode_history_obj[cap_mode_name][element], 
                    $(element).css(Object.keys(styles[element])));
                }
            }
        }
        else 
    ...
    function resolve_parallel_all_argument(exe) 
    {
        ...
        for (var argument_index = 0; argument_index < exe.arguments.length; argument_index++) 
        {
            ...
            exe.resolve_series_one_argument[argument_index].callback = get_input_callback_func(exe.resolve_series_one_argument[argument_index].obj, 
            true);
    ...
    function resolve_parallel_all_mode_in_one_obj(exe) 
    {
        ...
        var all_mode_name_array = (typeof (all_mode_in_one_argument['obj'][exe.element_name]) == 'object') ? all_mode_in_one_argument['obj'][exe.element_name] : new Array(all_mode_in_one_argument[exe.element_name]);
        ...
        exe.resolve_parallel_all_mode['max_pointer'] = all_mode_name_array.length - 1 exe.resolve_series_one_mode = new Array();
        ...
        for (var mode_index = 0; mode_index < all_mode_name_array.length; mode_index++) 
        {
            ...
            {
                case 'object':
                    exe.resolve_series_one_mode[mode_index].callback = get_input_callback_func( one_mode_data, 
                    true );
                ...
                case 'function':
                    ...
                    exe.resolve_series_one_mode[mode_index].callback = one_mode_data break;
                case 'string':
                    exe.resolve_series_one_mode[mode_index].styles = [{}];
                    exe.resolve_series_one_mode[mode_index].styles[ 0 ][ exe.element_name ] = one_mode_data exe.resolve_series_one_mode[mode_index].callback = get_input_callback_func( one_mode_data, 
                    true );
                    break;
            }
            ...
            for (var element in styles) 
            {
                ...
                switch ( effect_way )
                {
                    case 'class':
                        ...
                        $( element ).removeClass( remove_class.join( ' ' ) ) .addClass( add_class.join( ' ' ) );
                        break;
                    case 'object':
                        if ( typeof ( styles[element] ) == 'function' )
                        {
    ...
    function set_css_rules(rules, styleSheetsNum, rec_flg) 
    {
        ...
        for (var style_num = 0; style_num < rules.length; style_num++) 
        {
            ...
            {
                var media = (rules[style_num]['parentStyleSheet']['media']['mediaText'] != '') ? rules[style_num]['parentStyleSheet']['media']['mediaText'] : 'all';
            }
            else 
            {
                var media = (rules[style_num]['parentRule']['media']['mediaText'] != '') ? rules[style_num]['parentRule']['media']['mediaText'] : 'all';
            }
            if (fm.style[media] == undefined) {
                fm.style[media] = {};
            }
    ...
    function set_page(rules, media, style_num, type) 
    {
        ...
        {
            fm.style[media]['@page'][rules[style_num]['style'][i]] = rules[style_num]['style'][rules[style_num]['style'][i]];
        }
    }
    ...
    function set_style(rules, media, style_num) 
    {
        ...
        for (var i = 0; i < rules[style_num]['style'].length; i++) 
        {
            ...
            if ( mode_name_start_index > 0 )
            {
                ...
                {
                    var _set_mode_bank = {};
                    _set_mode_bank[ rules[style_num]['style'][i] ] = rules[style_num]['style'][rules[style_num]['style'][i]] set_mode_banks.push( _set_mode_bank );
                    //{'start_mode':{
                    //	'style' : [{}],
                    //	'sync'  : true,
                    //	'fn'    : function(){}
            ...
            fm.style[media][rules[style_num]['selectorText']][rules[style_num]['style'][i]] = rules[style_num]['style'][rules[style_num]['style'][i]];
    ...
    function set_keyframe(rules, media, style_num) 
    {
        ...
        for (var i = 0; i < rules[style_num][dig_key].length; i++) 
        {
            ...
            for (var j = 0; j < rules[style_num][dig_key][i]['style'].length; j++) 
            {
                ...
                fm.style[media][key_name][rules[style_num]['name']][rules[style_num][dig_key][i]['keyText']][rules[style_num][dig_key][i]['style'][j]] = rules[style_num][dig_key][i]['style'][rules[style_num][dig_key][i]['style'][j]];
            }
        }
    }
    function size( obj )
